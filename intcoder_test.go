...
func TestChunkIntCoder(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// 1 chunk, chunk-0 length 1, value 3, but compressed
			expected: []byte{
				0x1, 0xe, 0x28, 0xb5, 0x2f, 0xfd, 0x4, 0x0, 0x9,
				0x0, 0x0, 0x3, 0xb6, 0x4b, 0x1f, 0xbc,
			},
		},
		{
			maxDocNum: 1,
			...
			// 2 chunks, chunk-0 offset 1, chunk-1 offset 2, value 3, value 7, but compressed
			expected: []byte{
				0x2, 0xe, 0x1c, 0x28, 0xb5, 0x2f, 0xfd, 0x4, 0x0, 0x9,
				0x0, 0x0, 0x3, 0xb6, 0x4b, 0x1f, 0xbc, 0x28, 0xb5, 0x2f, 0xfd, 0x4, 0x0, 0x9,
				0x0, 0x0, 0x7, 0xb7, 0xbb, 0x58, 0xe8,
			},
		},
	}
...
var chunkReadBoundaryFromOffsetTests = []struct {
	chunkNumber   int
	offsets       []uint64
...
func TestChunkReadBoundaryFromOffsets(t *testing.T) {
	for i, test := range chunkReadBoundaryFromOffsetTests {
